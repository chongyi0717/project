...
int main()
{
    ...
    while(wipfile.good()){
        ...
        if(m!=0 && m<=JOB_NUMBER)
        {
            ...
            job_buffer->qty=stof(res1.at(3));  
            job_buffer->arrival_time=stof(res1.at(5));
    ...
    ifstream eqpfile(EQP_RECIPE_FILE);
     while(eqpfile.good()){
        ...
        if(m!=0 && m<=JOB_NUMBER)
        {
            ...
            EQP_buff->process_time=stof(res1.at(2));
        ...
        for(int n=0;n<CHO.size();n++)
        {
                ...
                for(int j=0;j<=job.at(i).machine_can_select.size();j++)
                {
                    ...
                    if(job.at(i).select>=buffer_for_adding_to_one-buffer_of_number_for_add && job.at(i).select<=buffer_for_adding_to_one)
                    {
                        ...
                        for(int k=0;k<EQP.size();k++)
                        {
                            ...
                            {
                                job.at(i).process_time=EQP.at(k).process_time/(float)25*(float)job.at(i).qty;
                                //cout<<job.at(i).job_name<<" "<<job.at(i).machine_selected<<" "<<EQP.at(k).process_time<<" "<<job.at(i).qty<<" "<<job.at(i).process_time<<endl;
                                break; 
                            }
            ...
            for(int i=0;i<machine.size();i++)
            {
                ...
                for(int j=0;j<machine.at(i).job_in_machine.size()-1;j++)
                {
                    ...
                    {
                        machine.at(i).set_up.push_back(setuptime[stoi(machine.at(i).job_in_machine.at(j).job_name)-1][stoi(machine.at(i).job_in_machine.at(j+1).job_name)-1]);
                        machine.at(i).arrive.push_back(0);
                        // machine.at(i).job_in_machine.at(j).process_time+=(float)setuptime[stoi(machine.at(i).job_in_machine.at(j).job_name)-1][stoi(machine.at(i).job_in_machine.at(j+1).job_name)-1];
                        //cout<<setuptime[stoi(machine.at(i).job_in_machine.at(j).job_name)-1][stoi(machine.at(i).job_in_machine.at(j+1).job_name)-1]<<endl;
                    }
                    else if(machine.at(i).job_in_machine.at(j).arrival_time-machine.at(i).makespan<=10)
                    {
                        machine.at(i).arrive.push_back((machine.at(i).job_in_machine.at(j).arrival_time-machine.at(i).makespan));
                        machine.at(i).set_up.push_back(setuptime[stoi(machine.at(i).job_in_machine.at(j).job_name)-1][stoi(machine.at(i).job_in_machine.at(j+1).job_name)-1]-(machine.at(i).job_in_machine.at(j).arrival_time-machine.at(i).makespan));
                    }
                    else
                    {
                        machine.at(i).set_up.push_back(0);
                        machine.at(i).arrive.push_back((machine.at(i).job_in_machine.at(j).arrival_time-machine.at(i).makespan));
                    ...
                    machine.at(i).makespan+=machine.at(i).set_up.at(j);
                    machine.at(i).makespan+=machine.at(i).arrive.at(j);
                }
                machine.at(i).makespan+=machine.at(i).job_in_machine.at(machine.at(i).job_in_machine.size()-1).process_time;
                machine.at(i).set_up.push_back(0);
                machine.at(i).arrive.push_back(0);
                machine.at(i).makespan+=machine.at(i).set_up.at(machine.at(i).job_in_machine.size()-1);
                machine.at(i).makespan+=machine.at(i).arrive.at(machine.at(i).job_in_machine.size()-1);
                //cout<<endl;
            }
    ...
    if(stof(text)>CHO.at(0).makespan)
    {
        ofstream smallestout("smallest_makespan.txt");
        smallestout<<CHO.at(0).makespan<<endl<<"Choromosome Number = "<<CHOROMOSOME_NUMBER<<endl<<"Iterative Number = "<<ITERATE_NUMBER<<endl;
    ...
    for(int i=0;i<CHO.at(0).machine.size();i++)
    {
        ...
        for(int j=0;j<CHO.at(0).machine.at(i).job_in_machine.size();j++)
        {            
            cout<<CHO.at(0).machine.at(i).set_up.at(j)<<" "<<CHO.at(0).machine.at(i).arrive.at(j)<<" ";
            cout<<CHO.at(0).machine.at(i).job_in_machine.at(j).job_name<<"("<<CHO.at(0).machine.at(i).job_in_machine.at(j).process_time<<","<<CHO.at(0).machine.at(i).job_in_machine.at(j).qty<<") ";
        }
        // for(int j=0;j<CHO.at(0).machine.at(i).set_up.size();j++)
        //     cout<<CHO.at(0).machine.at(i).set_up.at(j)<<" "<<CHO.at(0).machine.at(i).arrive.at(j)<<" ";
